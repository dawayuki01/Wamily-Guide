        ...
        if (item.url && item.url.startsWith('http')) {
          try {
            ...
            if (res.ok || res.status === 405 || res.status === 403) {
              // 405: HEAD非対応、403: ログイン必要 → URLは存在する
              validated.push(item);
            } else {
              console.warn(`    ⚠  URL無効 (${res.status}): ${item.name} → ${item.url}`);
            }
          } catch (e) {
            console.warn(`    ⚠  URL接続失敗: ${item.name} → ${item.url} (${e.message})`);
          }
        } else {
          console.warn(`    ⚠  URL未記入: ${item.name} → スキップ`);
        }
      }
      return validated;
    }

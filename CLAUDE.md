...
### 最優先：サイト導線・表示確認 + デザイン・言葉のブラッシュアップ
- **方針**: 早期アウトプットを目指す。やりすぎない。対話しながら進める。
- トップページ → 各国ページの導線チェック
- 全機能（スポット・地図・イベント・キュレーション・フィード）の表示確認
- デザイン面の微調整（レイアウト・色・余白など）
- 言葉のトーン統一・ブラッシュアップ
- WAMILY_DESIGN_SPEC.md との整合性チェック

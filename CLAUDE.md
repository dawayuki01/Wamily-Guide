...
- Google Analytics 4（GA4）でアクセス解析（測定ID: G-H96RGKM0MJ）
- GA4 Data API（googleapis）で週次レポート自動取得
...
| ホストDB | 2c4a0503-2191-4d46-8fa7-84a30209227a | ホスト管理（公開/非公開情報） |
| ダッシュボード | 33c088f5-0b07-8157-be5f-e1364759adf5 | 週次集計ページ（自動更新） |
| 参謀室DB | NOTION_STRATEGY_DB_ID（GitHub Secrets） | 月次レポート・壁打ちログ |
| ウォッチリストDB | NOTION_WATCHLIST_DB_ID（GitHub Secrets） | 業界ウォッチ対象管理 |
| Instagram投稿DB | NOTION_INSTAGRAM_DB_ID（GitHub Secrets） | Instagram投稿・アナリティクス管理 |

### Google Cloud Console（プロジェクト: sawady-twitter）
| APIキー名 | 用途 | 制限 |
|---|---|---|
| Maps Platform API Key | サーバー側＆ブラウザ側兼用 | Maps JS API + Places API (New) + 33個のAPI / 制限なし |

※ 旧キー（Wamily-Places, Wamily-Maps-Frontend）は廃止。Maps Platform API Key に統一済み。
...
| GA4_PROPERTY_ID | GA4 プロパティID（531805176） |
...
- draft: ガイドブック一覧・カルーセルに非表示。URL直打ちでアクセス可能
- public: サイトに表示される
- 新規追加: `node scripts/add-country.js` または手動で countries.json + HTML + データファイル作成
- 公開: countries.json の status を `"public"` に変更してcommit & push
...
| events-{slug}.json | TimeOut RSS(London) + Claude AI(他10カ国) | 各国イベント・季節アクティビティ |
| newsletter-history.json | 週刊メルマガ | 配信済み記事URL履歴（重複防止） |
| hosts.json | Notion ホストDB | アクティブホスト公開情報 |
...
| 6 | Wamily AI チーム組成 | ✅ Phase 1 完了（パトロール・コンテンツ・メルマガ）✅ Phase 2 完了（コミュニティ部 user/host）✅ Phase 3 完了（SNS部・参謀室・ガイドブック編集部） |
| 7 | サイト公開 | 上記完了後 |
...
| 場所 | URL |
|---|---|
| サイト本番 | https://wamily.jp/ |
| サイト本番（旧URL） | https://dawayuki01.github.io/Wamily-Guide/（wamily.jpにリダイレクト） |
...
| Notionダッシュボード | https://notion.so/33c088f50b078157be5fe1364759adf5 |
| Notion Integration設定 | https://www.notion.so/profile/integrations |
| GitHub Actions（コミュニティ） | https://github.com/dawayuki01/Wamily-Guide/actions/workflows/community.yml |
| GitHub Actions（参謀室） | https://github.com/dawayuki01/Wamily-Guide/actions/workflows/strategy.yml |

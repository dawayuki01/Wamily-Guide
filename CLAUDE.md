...
## 4月1日 後半の作業記録（デザイン・導線）

### 完了した作業
12. **回遊ナビ（国カルーセル）追加**
    - 全10カ国ページのフッター上に「📚 他の国のガイドブック」カルーセルを追加
    - 現在の国を除くランダム4カ国を表示（横スクロール対応）
    - data-loader.js に loadCountryCarousel() 追加

13. **デザイン微調整（CSS）**
    - セクション区切りをドット装飾（● ● ●）に変更
    - スポットカード・イベントカードにホバーエフェクト追加
    - タブナビにアクティブ下線アニメーション追加
    - パルスドットのアニメーション強調

14. **ロンドンページに浮遊イラスト装飾追加**
    - ビッグベン・ダブルデッカーバス・紅茶・ベビーカー・飛行機のSVG
    - float-drift アニメーションでゆっくり浮遊
    - モバイルでは非表示

15. **言葉のブラッシュアップ（初手）**
    - スポット確認表示を「✓ サワディー確認済み（2026年3月）」に変更

### デザイン方針の確定（ユーザーとの対話結果）
- **タブ構造は残す**（自分で探す体験として有効）
- **各タブの中身をエディトリアル化**する（写真大きく、余白広く、語り口調）
- **体験が良くなるならやりすぎてOK**（「やりすぎない」はサワディーの主役感を出しすぎない意味）
- **参考**: タテヤ観光案内書（https://www.glctateya.com/tourism/）のエディトリアル型レイアウト
- **現在の問題**: ページが「データベースUI」になっている → 「雑誌のような体験」に変えたい
- **ガイドブックの世界地図**: SVGの限界あり → Canva等でイラスト地図を作成したい
- **言葉**: サワディーが叩き台を作り、Claudeがブラッシュアップする方式もOK

---

## 次にやること（次回セッション）

### 最優先：ロンドンページのエディトリアル化リニューアル
- タブ1「ロンドンについて」: 写真＋推薦文スタイル。ポイントカードをエディトリアル型に
- タブ2「行く前に」: アコーディオン内にサワディーの体験談を追加
- タブ3「現地で楽しむ」: スポットカードに推薦者コメント＋写真スロット追加
- タブ4「旅のバトン」: 投稿フォーム的CTA強化
- 全体: セクション間に大きな写真を挟む。余白を広く。フォントサイズにメリハリ
- **ロンドンで完成 → 他9カ国に展開**

### ガイドブック世界地図のリニューアル
- SVG世界地図 → イラスト地図に差し替え（Canva等で作成）
- 手作り感・ワクワク感のある地図表現

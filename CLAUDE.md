...
### 3. 各国 Google マップのリッチ化 ← 次はここ

**方針：Google Maps の保存済みスポット → Notion → サイト自動反映**

サワディーの個人Googleアカウント（dawasawada@）に保存済みの「行ってよかった場所」ピンを一括で取り込む。

```
① Google Takeout で保存済みスポットをエクスポート（GeoJSON）
② スクリプト（scripts/import-saved-places.js）で Notion スポットDB に一括投入
   - ステータス：「候補」で投入
   - 国の自動判定（座標 → 国スラッグ）
   - カテゴリは仮で振る（後でサワディーが修正可）
③ サワディーが Notion のビューで確認 → 「公開」に変更
④ 毎日の fetch-notion.js で自動反映
```

**作るもの：**
- `scripts/import-saved-places.js`（Google Takeout GeoJSON → Notion スポットDB 一括投入）
- Notion スポットDB に「候補」ステータスを追加
- Notion に「候補一覧」ビューを作成（国別フィルター付き）

**サワディーがやること：**
- Google Takeout から「保存した場所」をエクスポート（takeout.google.com）
- Notion 上で候補を見て「公開」にするだけ
...
```
① Claude API で各国の「子連れ × 海外」YouTube / ブログ候補を自動生成
② Notion キュレーションDB に「候補」ステータスで投入
   - タイトル・URL・おすすめ理由付き
   - リンク生存確認を自動チェック（死んだリンクは除外）
③ サワディーが Notion 上で ○ / × をつける
④ ○ のものだけ fetch-notion.js でサイトに反映
```

**作るもの：**
- `scripts/generate-curation.js`（Claude API → キュレーション候補生成 + Notion投入）
- リンク生存チェック機能（HTTP HEAD で確認）
- Notion キュレーションDB に「候補」ステータス + 「候補一覧」ビュー追加

**サワディーがやること：**
- Notion でキュレーション候補を見て ○ / × をつけるだけ

### 5. メルマガ
- Wamily の旅情報・更新をメールで届ける仕組み
- 配信サービスの選定（Mailchimp / Buttondown / Resend 等）
- フレンズページに登録フォーム設置
- テンプレートデザイン + 配信コンテンツ設計
